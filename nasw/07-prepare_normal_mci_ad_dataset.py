...
def expected_audio_names(json_path, data, audio_kind):
    records = file_records(data)
    preference = ["raw", "fine", "init"] if audio_kind == "auto" else [audio_kind, "raw", "fine", "init"]
    names = []
    seen = set()
    ...
    for kind in preference:
        ...
        for record_key in AUDIO_KIND_KEYS[kind]:
            ...
            for value in (file_name, file_path):
                ...
                names.append(name)
                if file_ext and not Path(name).suffix:
                    names.append(f"{name}.{file_ext}")

    names.extend(candidate.name for candidate in fallback_audio_candidates(json_path))
    unique_names = []
    for name in names:
...
def source_root_candidates(source_root):
    requested = Path(source_root).expanduser()
    candidates = [requested]
    if requested.name != "dataset":
        candidates.append(requested / "dataset")
    candidates.extend(
        [
    ...
    roots = unique_existing_dirs(candidates)
    return roots or [requested]


def paper_type_roots(source_root, paper_types):
    roots = []
    for paper_type in paper_types:
        ...
        if not found and source_root.exists():
            found = [
                path
...
def discover_source_jsons(source_root, paper_types):
    source_roots = source_root_candidates(source_root)
    paper_roots = []
    for root in source_roots:
        paper_roots.extend(paper_type_roots(root, paper_types))
    paper_roots = unique_existing_dirs(paper_roots)

    json_roots = paper_roots or source_roots
    if not paper_roots:
        print(
            "warning: IB-APPS/CERAD-K/SNSB-II folders were not found directly; "
            "scanning all JSON files under source root candidates."
        )
    ...
    return json_paths, source_roots, json_roots


def build_audio_index(search_roots):
    index = {}
...
def choose_audio_from_index(json_path, data, audio_kind, audio_index):
    for name in expected_audio_names(json_path, data, audio_kind):
        path = audio_index.get(name)
        if path:
            suffix = path.stem.rsplit("_", 1)[-1]
            kind = {"R": "raw", "F": "fine", "I": "init"}.get(suffix, "indexed")
            return path, kind
    return None, None
...
def write_preprocessing_config(path, args, counts):
    config = {
        ...
        "source_roots_scanned": getattr(args, "source_roots_scanned", []),
        "json_roots_scanned": getattr(args, "json_roots_scanned", []),
        "metadata_csv": str(args.metadata_csv or ""),
        "audio_kind": args.audio_kind,
        "recursive_audio_search": args.recursive_audio_search,
...
def rows_from_source(args):
    rows = []
    json_paths, source_roots, json_roots = discover_source_jsons(args.source_root, args.paper_types)
    args.source_roots_scanned = [str(path) for path in source_roots]
    args.json_roots_scanned = [str(path) for path in json_roots]

    print("source_roots_scanned:")
    for root in source_roots:
        print(f"  - {root}")
    print("json_roots_scanned:")
    for root in json_roots:
        print(f"  - {root}")
    ...
    for json_path in json_paths:
        ...
        if args.recursive_audio_search and (not audio_path or not audio_path.exists()):
            if audio_index is None:
                print("building recursive audio index for irregular dataset layout...")
                audio_index = build_audio_index(source_roots)
            indexed_audio_path, indexed_audio_kind = choose_audio_from_index(
                json_path, data, args.audio_kind, audio_index
            )
            if indexed_audio_path:
                audio_path, audio_kind = indexed_audio_path, indexed_audio_kind
...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="recursive_audio_search",
        action="store_false",
        help=(
            "Disable the fallback that searches source roots for matching audio filenames "
            "when JSON and audio files are stored in separate folders."
        ),
    )
    parser.set_defaults(recursive_audio_search=True)
